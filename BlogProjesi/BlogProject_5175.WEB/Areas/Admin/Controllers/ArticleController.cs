...
namespace BlogProject_5175.WEB.Areas.Admin.Controllers
{
    ...
    public class ArticleController : Controller
    {
        ...
        public IActionResult List(string Ara) // tüm makaleleri gösterir
        {
            ...
            {
                articleList = articleList.Where(x => x.Title.ToString().ToLower().Contains(Ara.ToLower())).ToList();
            }
        ...
        public IActionResult UnconfirmedArticle() // Pasif olanların listesi alınarak adminin onayına sunuldu
        {
            var articleList = articleRepository.GetByDefaults
               (
                   selector: a => new GetArticleVM()
        ...
        [HttpPost]
        public IActionResult UnconfirmedArticle(string button, int id)
        {
            Article article = articleRepository.GetDefault(a => a.ID == id);
            if (button == "confirm")
            {
                ...
                articleRepository.Update(article);
                return RedirectToAction("UnconfirmedArticle");
            }
            else
            {
                List<Category> kategoriler = categoryRepository.GetDefaults(x => x.Statu != Statu.Passive); //kategorilerin listesi alınıyor
                foreach (var item in kategoriler.ToList()) //silinecek makalenin kategorileri bulunuyor ve ArticleCategory'si databaseden siliniyor
                {
                    ArticleCategory articleCategoryList = articleCategoryRepository.GetDefault(x => x.ArticleID == article.ID && x.CategoryID == item.ID);
                    if (articleCategoryList != null)
                    {
                        articleCategoryRepository.Delete(articleCategoryList);
                    }
                }
                articleRepository.DeleteFromDatabase(article);
            }
            return RedirectToAction("UnconfirmedArticle");
        }
